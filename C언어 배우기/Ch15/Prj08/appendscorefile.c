/*
	솔루션 / 프로젝트 / 소스파일: Ch15 / Prj08 / appendscorefile.c
...
int main()
{
	...
	if (readcnt == 1)
	{
		...
		//파일의 마지막에서 마지막 학생을 읽기 위핸 한 학생만큼 뒤로 이동
		fseek(f, -offset, SEEK_END);

		//fseek(f, -offset, SEEK_END);		//파일의 현재 포인터에서 한 학생만큼 뒤로 이동
		fread(&score, sizeof(pscore), 1, f);	//마지막 학생을 읽음

		//제일 마지막 자료의 번호를 찾기 위해서
		cnt = score.number;
		printf("\n제일 마지막 번호가 %d번 입니다. \n\n", cnt);
	}
	fseek(f, 0L, SEEK_END);
